...
class OverpassParser::TestVisitor < Minitest::Test
  def test_selector
    ...
    assert_equal([{:type=>:request, :queries=>[{:type=>:query_object, :set=>nil, :object_type=>"node", :selectors=>[{:type=>:selector, :value=>"primary", :key=>"highway", :operator=>"="}], :filter=>[]}]}], tree)
  end

  def test_selector_set
    tree = OverpassParser.tree('node._[highway=primary];')
    assert_equal([{:type=>:request, :queries=>[{:type=>:query_object, :set=>'._', :object_type=>"node", :selectors=>[{:type=>:selector, :value=>"primary", :key=>"highway", :operator=>"="}], :filter=>[]}]}], tree)
  end
