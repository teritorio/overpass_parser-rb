...
module OverpassParser::Nodes
  class TestFilters < Minitest::Test
    ...
    def test_matches_to_sql
      ...
      assert_equal("ST_Intersects(geom, (SELECT geom FROM a))", parse("(area.a").to_sql(q))
      assert_equal("ST_Within(geom, (SELECT geom FROM a, 12.3)", parse("(around.a:12.3)").to_sql(q))
    end
  end
end
